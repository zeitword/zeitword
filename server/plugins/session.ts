...
export default defineNitroPlugin(() => {
  ...
  sessionHooks.hook("fetch", async (session, event) => {
    ...
    const [existing] = await useDrizzle()
      .select()
      .from(sessions)
      .where(and(eq(sessions.token, session.secure.sessionToken), isNull(sessions.deletedAt)))
    if (!existing) throw createError({ statusCode: 401, message: "Invalid token" })

    // Check that the createdAt date is max 30 days ago
    const createdAt = new Date(existing.createdAt)
    const maxAge = new Date(createdAt.getTime() - 1000 * 60 * 60 * 24 * 30)
