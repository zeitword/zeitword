...
export function getStoredToken(apiUrl: string): string | null {
  try {
    if (!fs.existsSync(TOKEN_FILE)) return null
    const data = JSON.parse(fs.readFileSync(TOKEN_FILE, "utf-8")) as StoredAuth
    if (data.apiUrl !== apiUrl) return null
    return data.token
  } catch {
    return null
  }
...
function storeToken(token: string, apiUrl: string): void {
 ...
 */
export async function authenticateViaBrowser(apiUrl: string): Promise<string> {
